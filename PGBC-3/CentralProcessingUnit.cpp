...
void CentralProcessingUnit::rlcAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);

	(value >> 7) ? setFlagCY() : clearFlagCY();
	clearFlagN();
	clearFlagH();

	value = (value << 1) | (value >> 7);

	(value == 0) ? setFlagZ() : clearFlagZ();

	setByte(adderss, value);
}
...
void CentralProcessingUnit::rrcAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);

	(value & 0x01) ? setFlagCY() : clearFlagCY();
	clearFlagN();
	clearFlagH();
...
void CentralProcessingUnit::rlAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);

	unsigned char tempBit = value & 0x80;

	value = (value << 1) | getFlagCY();
	(tempBit > 0) ? setFlagCY() : clearFlagCY();
	clearFlagN();
	clearFlagH();
	(value == 0) ? setFlagZ() : clearFlagZ();
...
void CentralProcessingUnit::rrAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);

	unsigned char tempBit = value & 0x01;

	value = (value >> 1) | (getFlagCY() << 7);
	(tempBit > 0) ? setFlagCY() : clearFlagCY();
	clearFlagN();
	clearFlagH();
	(value == 0) ? setFlagZ() : clearFlagZ();
...
void CentralProcessingUnit::slaAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);

	(value & 0x80) ? setFlagCY() : clearFlagCY();
	clearFlagN();
	clearFlagH();
...
void CentralProcessingUnit::sraAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);
...
void CentralProcessingUnit::swapAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);
...
void CentralProcessingUnit::srlAdr(unsigned short & adderss)
{
	unsigned char value = getByte(adderss);
...
void CentralProcessingUnit::resBitAdr(unsigned short & adderss, const unsigned char bit)
{
	unsigned char value = getByte(adderss);
...
void CentralProcessingUnit::setBitAdr(unsigned short & adderss, const unsigned char bit)
{
	unsigned char value = getByte(adderss);
...
void CentralProcessingUnit::testBitAdr(unsigned short & adderss, const unsigned char bit)
{
	unsigned char value = getByte(adderss);

	((value & (0x1 << bit)) == 0) ? setFlagZ() : clearFlagZ();
	clearFlagN();
	setFlagH();
...
StateCPU CentralProcessingUnit::getUpdate()
{
	StateCPU status;

	status.IME = IME;
	status.CurrentClock = CurrentClock;
	status.SP = SP;
	status.PC = PC;
	status.AF = AF;
	status.BC = BC;
