...
void MemoryManagementUnit::MBC_1(unsigned short address, unsigned char value)
{
	if (address < 0x2000)
	{
		/* Enable/Disable RAM - Not Implemented */
	}
	else if (address < 0x4000)
	{
		selected_ROM_bank = value & 0x1F;
		if (selected_ROM_bank == 0x0  && 
			selected_ROM_bank == 0x20 &&
			selected_ROM_bank == 0x40 && 
	...
	else if (address < 0x6000)
	{
		...
		{
			selected_ROM_bank = (selected_ROM_bank & 0x1F) | ((value & 0x03) << 5);
		}
		else
		{
			selected_ERAM_bank = value & 0x03;
		}
	}
	else
	{
...
void MemoryManagementUnit::initMMU(char * cartridgeBase)
{
	...
	{
	case None:				ERAM_banks = 0;	 break;
	case S_2_KB_1_banks:	ERAM_banks = 1;	 break;
	case S_8_KB_1_banks:	ERAM_banks = 1;	 break;
	case S_32_KB_4_banks:	ERAM_banks = 4;	 break;
	case S_128_KB_16_banks:	ERAM_banks = 16; break;
	}
	...
	memorySize += ERAM_SIZE * ERAM_banks;
	...
	if (ERAM_banks > 0)
	{
		for (unsigned char count = 0; count < ERAM_banks; count++)
		{
	...
	{
		baseERAM_n.push_back(baseBGD2.back() + BGD2_SIZE);
	}
...
void MemoryManagementUnit::write(unsigned short address, unsigned char value)
{
	...
	{
		baseERAM_n.at(selected_ERAM_bank)[address - ERAM_OFFSET] = value;
	}
	...
	else if (address < ZP_OFFSET)
	{
		...
		if ((address - IOREG_OFFSET) == 0x50)
		{
			runningBootCode = !value;
		}
	}
	else
	{
...
unsigned char MemoryManagementUnit::read(unsigned short address)
{
	...
	{
		value = baseROM_n.at(selected_ROM_bank)[address - ROM_Bn_OFFSET];
	}
	else if (address < BGD1_OFFSET)
	{
	...
	{
		value = baseERAM_n.at(selected_ERAM_bank)[address - ERAM_OFFSET];
	}
	else if (address < IROM_Bn_OFFSET)
	{
...
unsigned char* MemoryManagementUnit::getVRAMdebugInfo(unsigned char bank)
{
	return baseCRAM.at(bank);
}
